...
export async function POST(req: Request) {
  ...
  if (!body || typeof body.cart_value !== "number") {
  ...
  const rows = await sql`
    ...
    VALUES (${body.user_email ?? "unknown@example.com"}, ${body.user_phone ?? ""}, ${body.cart_value}, ${JSON.stringify(body.items ?? [])}::jsonb, ${body.customer_type ?? "New"}, 'Pending')
    RETURNING id
  `;
